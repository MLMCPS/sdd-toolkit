...
You are bootstrapping the **sdd-toolkit** into the current repository. This is a **learn-first**
...
`docs/`, or an existing spec/RFC/ADR practice, stop and recommend **`/sdd-adopt`** instead — it
...
reading code. So spawn **sdd-scanner** three times **in a single message**, so the three run
...
Write these so the agents (`coder`, `sdd-*`) have durable project memory. Cite real `file:line`;
...
6. **`docs/ESTATE.md` — only for a multi-service estate.** If this repo is part of a larger estate
   ...
   **`/sdd-estate`**, which scans the peer repos and fills
...
11. **Knowledge-layer CI gate:** copy `${CLAUDE_PLUGIN_ROOT}/templates/ci/knowledge-check.mjs` to
    ...
    longer exists — the mechanical half of `/sdd-doctor`, which is the half worth automating.
...
12. **Optional MCP server:** mention that `${CLAUDE_PLUGIN_ROOT}/templates/mcp/.mcp.json` wires this
...
13. **Optional hooks:** mention that `${CLAUDE_PLUGIN_ROOT}/templates/hooks/settings.hooks.example.json`
...
14. Report what was created/skipped/merged, and what the human should review — especially
...
The agents (`coder`, `sdd-spec-author`, `sdd-spec-reviewer`, `sdd-developer`, `sdd-reviewer`,
